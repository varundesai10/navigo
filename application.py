...
def handleFileUpload():
    if 'agentsfile' in request.files:
        ...
        if agentsfile.filename != '':            
            agentsfile.save(os.path.join('uploadsfolder/', 'agents.txt'))
    ...
    if 'consumersfile' in request.files:
        ...
        if consumersfile.filename != '':            
            consumersfile.save(os.path.join('uploadsfolder/', 'locations.txt'))
    return redirect(url_for('script'))

@app.route("/script")
def script():
    os.system('python3 master_file.py uploadsfolder/locations.txt uploadsfolder/agents.txt templates/finalroutes.html > outputlog.txt')
    os.system('rm -rf agentdata.json finalroutes.json distances.json')
    return render_template("finalroutes.html")

@app.route("/specificroute/<id>")
def spicificroute(id):
